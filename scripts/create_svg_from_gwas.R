svg_from_ld_block <- function() {
  files <- Sys.glob(glue::glue('/Users/wt23152/Documents/Projects/scratch/021/data/svgs/blocks/*.tsv.gz'))
  plot_height <- 200
  plot_width <- 1000
  
  lapply(files, function(file) {
    # Read and process the LD block data
    ld_block <- vroom::vroom(file) |>
      dplyr::mutate(CHR = as.numeric(CHR), Z = Z) |>
      dplyr::filter(!is.na(CHR)) |>
      dplyr::arrange(CHR, BP)
    
    # Process data with higher resolution bins (10kb instead of 100kb)
    ld_data <- ld_block |>
      dplyr::filter(!is.na(Z)) |>
      dplyr::mutate(bin = floor(BP/10000)) |>  # 10kb bins for higher resolution
    ...
    # Create area data with larger bins (100kb instead of 1Mb)
    ld_area <- ld_data |>
      dplyr::mutate(bin = floor(BP/10000)) |>  # 100kb bins for area
      dplyr::group_by(CHR, bin) |>
      dplyr::summarise(
        BP = mean(BP),
        Z = max(Z),
    ...
    ld_area <- ld_area |>
      ...
      dplyr::group_modify(~{
        # Get the min and max BP for this chromosome
        min_bp <- min(.x$BP)
        max_bp <- max(.x$BP)
        
        # Create the proper sequence: Z=0 at start, data, Z=0 at end
        dplyr::bind_rows(
    ...
    # Create the plot with just the area
    ...
    block_name <- basename(file) |> stringr::str_replace("\\.tsv\\.gz$", "")
    
    # Save the plot
    ggplot2::ggsave(
      glue::glue("segment_{block_name}.svg"),
      p,
      width = plot_width/72,
...
svg_from_gwas <- function(name) {
  ...
  original_gwas <- vroom::vroom(glue::glue('/Users/wt23152/Documents/Projects/scratch/021/data/svgs/{name}.tsv.gz')) |>
  ...
  gwas_area <- gwas_area |>
    ...
    dplyr::group_modify(~{
      # Get the min and max BP_cumulative for this chromosome
  ...
  for (chr_num in unique(original_gwas$CHR)) {
    ...
    p_chr <- ggplot2::ggplot(chr_data, ggplot2::aes(x = BP_cumulative, y = LP, color = color, fill = color)) +
      ggplot2::geom_area(
        data = chr_area_data,
        ggplot2::aes(group = CHR),
        position = "identity"
    ...
    # Save individual chromosome SVG with same dimensions
    ggplot2::ggsave(
      glue::glue("manhattan_plot_{name}_chr{chr_num}.svg"),
      p_chr,
      width = plot_width/72,
      height = plot_height/72,
      units = "in",
