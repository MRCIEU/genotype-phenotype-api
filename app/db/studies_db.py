...
class StudiesDBClient:
    ...
    @log_performance
    def get_pathway_mappings_for_genes(self, gene_ids: List[int], source: str = None):
        if not gene_ids:
            return []
        query = "SELECT * FROM pathway_mappings WHERE gene_id IN (SELECT * FROM UNNEST(?))"
        params = [gene_ids]
        if source:
            query += " AND source = ?"
    ...
    @log_performance
    def get_pathway_sizes(self, term_ids: List[str], source: str = None):
        if not term_ids:
            return []
        query = "SELECT * FROM pathway_sizes WHERE term_id IN (SELECT * FROM UNNEST(?))"
        params = [term_ids]
        if source:
            query += " AND source = ?"
            params.append(source)
    ...
    @log_performance
    def get_all_pathway_mappings(self, source: str = None):
        query = "SELECT gene_id, term_id, source FROM pathway_mappings"
        params = []
        if source:
            query += " WHERE source = ?"
            params.append(source)
        query += " ORDER BY source, term_id, gene_id"
        return self.studies_conn.execute(query, params).fetchall()
