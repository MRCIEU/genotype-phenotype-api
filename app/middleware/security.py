    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        for param in request.path_params.values():
            ...
            if sanitized_param != param:
                return Response(
                    content="Invalid characters in request",
                    status_code=400
                )
        ...
        try:
            sanitized_query = await self.sanitize_query_params(query_params)
        except Exception:
            return Response(
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            try:
                ...
                async def receive():
                    return {"type": "http.request", "body": str(sanitized_body).encode()}
